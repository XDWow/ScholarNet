...
type AsyncSms struct {
	...
	// 重试的最大次数
	RetryMax int
	// 服务商重试策略:错误 failover,超时 failover
	Strategy string
	// 去重用的
	BizType string `gorm:`
	BizID   string
...
func (g *GORMAsyncSmsDAO) GetWaitingSMS(ctx context.Context) (AsyncSms, error) {
	...
	err := g.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		// 为了避开一些偶发性的失败，只找 30s 前的异步短信发送
		now := time.Now().UnixMilli()
		endTime := now - time.Minute.Milliseconds()/2
		err := tx.Clauses(clause.Locking{Strength: "UPDATE"}). // 对查到的这一条记录加写锁（悲观锁）
									Where("utime < ? and status = ?", endTime, asyncStatusWaiting).First(&s).Error
		// 对应sql语句：SELECT xx FROM xxx WHERE xx FOR UPDATE，锁住了
		if err != nil {
			return err
		}

		// 只要更新了更新时间，根据我们前面的规则，就不可能被别的节点抢占了，不会让别的节点卡在这里
